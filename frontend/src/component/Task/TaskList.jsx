...
const TaskList = ({ task, notify, setLoading, setError }) => {
    ...
    return (
        <>
            {popup && <TaskDetail openPopup={openPopup} closePopup={closePopup} task={task} notify={notify} setLoading={setLoading} setError={setError} />}
            <tr key={task._id} className="">
                ...
                <td className="py-3 bg-white-800 p-3 text-sm font-semibold tracking-wide">
                    <p onClick={openPopup} className="cursor-pointer text-orange hover:text-yellow-300">Open</p>
                </td>
            </tr>
        </>
    );
